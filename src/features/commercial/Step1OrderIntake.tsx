...
export function Step1OrderIntake({ request, onChange, onNext }: Step1OrderIntakeProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      {/* Quick RFQ Presets */}
      <div className="bg-[#0e1118] border border-[#1e2433] rounded-xl p-4 mb-6 shadow-md">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <button
            ...
            className={`p-3 rounded-lg border text-left transition-all cursor-pointer ${
              request.targetMarketId === 'DE_THG' && request.feedstockKey === 'manure'
                ? 'bg-cyan-950/40/60 border-cyan-500/50 text-cyan-200'
                : 'bg-[#08090d] border-[#1e2433] hover:border-[#2b3347] text-zinc-300'
            }`}
          >
            <div className="text-sm font-semibold mb-0.5">🇩🇪 DE THG Manure</div>
            <div className="font-mono text-[10px] text-zinc-400">10k MWh · Manure</div>
          </button>
          ...
          >
            ...
            <div className="font-mono text-[10px] text-zinc-400">15k MWh · Bio-waste</div>
          </button>
          ...
          >
            ...
            <div className="font-mono text-[10px] text-zinc-400">8k MWh · Agri-straw</div>
          </button>
          ...
          >
            ...
            <div className="font-mono text-[10px] text-zinc-400">5k MWh · Sewage</div>
          </button>
        </div>
      </div>

      {/* Main Order Form */}
      <div className="bg-[#0e1118] border border-[#1e2433] rounded-xl p-6 shadow-xl space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Buyer Market */}
          <div>
            <label className="block font-mono text-xs uppercase tracking-wider text-zinc-300 font-bold mb-2">
              1. Buyer Destination Market
            </label>
            <select
              value={request.targetMarketId}
              onChange={e => onChange({ targetMarketId: e.target.value })}
              className="w-full bg-[#08090d] border border-[#2b3347] rounded-lg px-3 py-2.5 font-mono text-sm text-zinc-100 focus:outline-hidden focus:border-cyan-500/50"
            >
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              <span className="absolute right-3 top-2.5 font-mono text-xs text-zinc-500 font-semibold">
                MWh
              </span>
            </div>
            <span className="font-mono text-[10px] text-zinc-500 mt-1 block">
        ...
        {/* Action Button */}
        <div className="pt-4 border-t border-[#1e2433] flex justify-end">
          <button
            type="button"
            onClick={onNext}
            className="w-full sm:w-auto px-8 py-3.5 rounded-lg bg-cyan-500 text-black font-bold hover:bg-cyan-500 text-stone-950 font-mono text-sm font-bold tracking-wide transition-all shadow-lg hover:shadow-teal-900/30 flex items-center justify-center gap-2 cursor-pointer"
          >
