  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      <div className="text-center mb-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-500/40 text-emerald-700 dark:text-emerald-300 font-mono text-xs font-semibold mb-3">
      ...
      {/* Main Container */}
      <div className="bg-white dark:bg-[#0e1118] border border-slate-200 dark:border-[#1e2433] rounded-xl overflow-hidden shadow-sm dark:shadow-xl mb-6">
        {/* Deal Ref & Action Bar */}
        <div className="p-4 bg-slate-50 dark:bg-[#08090d] border-b border-slate-200 dark:border-[#1e2433] flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="font-mono text-xs font-bold text-slate-700 dark:text-zinc-300">
              Reference: <span className="text-cyan-700 dark:text-cyan-400 font-semibold">{dealRef}</span>
            </span>
            <span className="font-mono text-[10px] bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 px-2 py-0.5 rounded font-semibold">
              EXECUTION READY
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleCopy}
              className="px-3 py-1.5 rounded-lg bg-white dark:bg-[#0e1118] hover:bg-slate-100 dark:hover:bg-[#141824] border border-slate-200 dark:border-[#2b3347] text-slate-800 dark:text-zinc-200 font-mono text-xs transition-colors flex items-center gap-1.5 cursor-pointer shadow-2xs"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-slate-200 dark:divide-stone-800 border-b border-slate-200 dark:border-[#1e2433] bg-slate-50/50 dark:bg-[#08090d]">
          <div className="p-5">
            <span className="font-mono text-[10px] text-slate-500 dark:text-zinc-400 uppercase tracking-wider block">
              Order Volume &amp; Tenor
            </span>
            <span className="font-mono text-xl font-bold text-slate-900 dark:text-zinc-100 block mt-1">
              {vol.toLocaleString()} MWh
            </span>
            <span className="font-mono text-xs text-cyan-700 dark:text-cyan-400 block mt-0.5">
          ...
          <div className="p-5">
            ...
            <span className="font-mono text-xl font-bold text-red-600 dark:text-red-300 block mt-1">
              €{totalDeliveredCostEur.toFixed(2)} / MWh
            </span>
            <span className="font-mono text-xs text-slate-500 dark:text-zinc-400 block mt-0.5">
              Total: €{Math.round(totalDealCostEur).toLocaleString()}
            </span>
          </div>

          <div className="p-5 bg-emerald-50/70 dark:bg-emerald-950/20">
            <span className="font-mono text-[10px] text-emerald-700 dark:text-emerald-400 uppercase tracking-wider block font-semibold">
              Total Net Deal Profit
            </span>
            <span className="font-mono text-2xl font-bold text-emerald-600 dark:text-emerald-300 block mt-1">
              {isProfitable ? '+' : ''}€{Math.round(totalDealProfitEur).toLocaleString()}
            </span>
            <span className="font-mono text-xs text-emerald-700 dark:text-emerald-400 block mt-0.5 font-medium">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-mono">
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#08090d] border border-slate-200 dark:border-[#1e2433]">
              <span className="text-[10px] uppercase text-slate-500 dark:text-zinc-500 block mb-1">Source Plant</span>
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#08090d] border border-slate-200 dark:border-[#1e2433]">
              <span className="text-[10px] uppercase text-slate-500 dark:text-zinc-500 block mb-1">Buyer Market</span>
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#08090d] border border-slate-200 dark:border-[#1e2433]">
              <span className="text-[10px] uppercase text-slate-500 dark:text-zinc-500 block mb-1">Feedstock &amp; Carbon Intensity</span>
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#08090d] border border-slate-200 dark:border-[#1e2433]">
              <span className="text-[10px] uppercase text-slate-500 dark:text-zinc-500 block mb-1">Logistics &amp; Chain of Custody</span>
          ...
          <div className="rounded-xl border border-slate-200 dark:border-[#1e2433] bg-white dark:bg-[#08090d] overflow-hidden font-mono text-xs shadow-2xs">
            <div className="p-3 bg-slate-100 dark:bg-[#0e1118] border-b border-slate-200 dark:border-[#1e2433] font-bold uppercase text-[11px] text-slate-800 dark:text-zinc-300">
            ...
            <table className="w-full text-left">
              <tbody className="divide-y divide-slate-200 dark:divide-stone-800/60 text-slate-700 dark:text-zinc-300">
                <tr>
                  <td className="p-3 text-slate-600 dark:text-zinc-400">1. Plant Gate Sourcing Cost</td>
                  <td className="p-3 text-right font-semibold text-slate-900 dark:text-zinc-200">€{plantGateEur.toFixed(2)}/MWh</td>
                  <td className="p-3 text-right text-slate-600 dark:text-zinc-400">€{Math.round(plantGateEur * vol).toLocaleString()}</td>
                </tr>
                <tr>
                  <td className="p-3 text-slate-600 dark:text-zinc-400">2. Grid Entry/Exit &amp; Transit Tariffs</td>
                  <td className="p-3 text-right font-semibold text-amber-600 dark:text-amber-300">€{gridLogisticsEur.toFixed(2)}/MWh</td>
                  <td className="p-3 text-right text-slate-600 dark:text-zinc-400">€{Math.round(gridLogisticsEur * vol).toLocaleString()}</td>
                </tr>
                <tr>
                  <td className="p-3 text-slate-600 dark:text-zinc-400">3. Mass Balance &amp; Proof of Sustainability</td>
                  <td className="p-3 text-right font-semibold text-slate-700 dark:text-zinc-300">€{certificationEur.toFixed(2)}/MWh</td>
                  <td className="p-3 text-right text-slate-600 dark:text-zinc-400">€{Math.round(certificationEur * vol).toLocaleString()}</td>
                </tr>
                <tr className="bg-slate-100/70 dark:bg-[#0e1118] font-semibold text-slate-900 dark:text-zinc-100">
                  ...
                  <td className="p-3 text-right text-red-600 dark:text-red-300">€{totalDeliveredCostEur.toFixed(2)}/MWh</td>
                  <td className="p-3 text-right text-red-600 dark:text-red-300">€{Math.round(totalDealCostEur).toLocaleString()}</td>
                </tr>
                <tr>
                  <td className="p-3 text-slate-600 dark:text-zinc-400">4. Wholesale Gas Offtake (TTF Index)</td>
                  <td className="p-3 text-right font-semibold text-slate-900 dark:text-zinc-200">€{gasIndexEur.toFixed(2)}/MWh</td>
                  <td className="p-3 text-right text-slate-600 dark:text-zinc-400">€{Math.round(gasIndexEur * vol).toLocaleString()}</td>
                </tr>
                <tr>
                  <td className="p-3 text-slate-600 dark:text-zinc-400">5. {opportunity.targetMarketName} Certificate Premium</td>
                  <td className="p-3 text-right font-bold text-cyan-700 dark:text-cyan-300">€{certificateValueEur.toFixed(2)}/MWh</td>
                  <td className="p-3 text-right text-cyan-700 dark:text-cyan-300">€{Math.round(certificateValueEur * vol).toLocaleString()}</td>
                </tr>
                <tr className="bg-slate-100/70 dark:bg-[#0e1118] font-semibold text-slate-900 dark:text-zinc-100">
                  <td className="p-3">Total Realizable Revenue (Credits)</td>
                  <td className="p-3 text-right text-cyan-700 dark:text-cyan-300">€{totalGrossRevenueEur.toFixed(2)}/MWh</td>
                  <td className="p-3 text-right text-cyan-700 dark:text-cyan-300">€{Math.round(totalDealRevenueEur).toLocaleString()}</td>
                </tr>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-slate-200 dark:border-[#1e2433]">
        <button
          type="button"
          onClick={onBack}
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={onReset}
            className="px-4 py-2.5 rounded-lg bg-white dark:bg-[#0e1118] hover:bg-slate-50 dark:hover:bg-[#141824] border border-slate-200 dark:border-[#2b3347] text-slate-700 dark:text-zinc-300 font-mono text-xs font-semibold transition-all flex items-center gap-2 cursor-pointer shadow-2xs"
          >
          ...
          <button
            ...
            className="px-6 py-2.5 rounded-lg bg-cyan-600 hover:bg-cyan-700 text-white dark:bg-cyan-500 dark:hover:bg-cyan-400 dark:text-stone-950 font-mono text-xs font-bold transition-all shadow-md flex items-center gap-2 cursor-pointer"
          >
